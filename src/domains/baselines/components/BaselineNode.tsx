      ...
      <div
        className={`flex items-center justify-end gap-5 h-[162px] ${
          showConnector
            ...
            : "before:absolute before:content-[''] before:inline-block before:w-[6px] before:h-[160px] before:bg-white before:right-[37px] before:-top-[138px] before:rounded-full after:absolute after:content-[''] after:inline-block after:w-[6px] after:h-[80px] after:bg-white after:right-[37px] after:top-[142px] after:rounded-full"
        }`}
      >
        {/* 연도와 이벤트 제목 표시 */}
        <div className="flex flex-col items-end gap-3">
          ...
          {/* 이벤트 제목 또는 기본 안내 텍스트 */}
          <div
            className={`relative text-[24px] text-white max-w-[210px] text-right line-clamp-1`}
          >
            {event?.eventTitle ? (
              event.eventTitle
            ) : (
        ...
        <div className="relative flex items-center" ref={innerRef}>
          ...
          {isTemp && !hasEvent && onDelete && (
            <button
              onClick={handleDelete}
              className="absolute -top-2 -right-2 w-5 h-5 bg-[#E76F51] text-white rounded-full flex items-center justify-center text-xs transition-colors z-20"
              title="노드 삭제"
            >
